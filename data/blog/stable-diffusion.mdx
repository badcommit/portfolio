...
Kind of egg and chicken problem. We need the real metric to back-propagate the loss. But the generation process will imply the real metric.
...
However, what should the output of this linear interpolation be? The loss is hard to define. Also, the linear interpolation highly depends on the result of the encoder, making it even harder to converge.

The second thought is that we wish that the low dimension space will follow a some kind of distribution. Each point of it will be a valid point.

The VAE tries to solve this problem by adding some `noise` to the low dimension space by assuming it is a normal distribution. Therefore during the training process, the decoder is forced to learn a more generalized mapping to the high dimension space.

the genius part of VAE is trying to let the low dimension to follow a normal distribution. By doing that, during the training process, it will become more robust when a new point.

$z_i \sim N(0,I)$

$q_\theta (z|x) = N(\mu_\theta(x), \sigma_\theta(x))$

zi is a sample drawed from a normal distribution in the low dimension space. We want
